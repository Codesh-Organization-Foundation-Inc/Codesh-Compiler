 ...
 */
class widening_cast_ast_node final : public op::assignment::cast_ast_node
{
    std::unique_ptr<primitive_type_ast_node> target_type;

public:
    widening_cast_ast_node(lexer::code_position code_position, std::unique_ptr<value_ast_node> value,
        std::unique_ptr<primitive_type_ast_node> target_type);
