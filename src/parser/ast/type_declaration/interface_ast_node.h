...
public:
    ...
    [[nodiscard]] const std::vector<std::unique_ptr<type::custom_type_ast_node>> &get_extended_interfaces() const;

    void add_method(std::unique_ptr<method::method_declaration_ast_node> method);
    [[nodiscard]] const std::deque<std::unique_ptr<method::method_declaration_ast_node>> &get_methods() const;
